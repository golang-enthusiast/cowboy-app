...
// LoadAppConfig reads configuration from the environment variables.
func LoadAppConfig() (config domain.AppConfig, _ error) {
...
// LoadCronConfig reads configuration from the environment variables.
func LoadCronConfig() (config domain.CronConfig, _ error) {
	if err := env.Parse(&config); err != nil {
		return config, fmt.Errorf("Failed to read configuration: %v", err)
	}
	return config, nil
